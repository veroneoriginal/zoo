# скрипт без django-окружения и без веб-сервера
...
data = {'max_age': 30}
# partial указывается, когда нужно обновить только некоторые поля объекта
serializer = FamilySerializer(updated_family, data=data, partial=True)

# partial НЕ указывается, когда осуществляется замена всех полей
